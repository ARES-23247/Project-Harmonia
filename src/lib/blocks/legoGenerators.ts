...
export function registerLegoGenerators() {
  pythonGenerator.forBlock["lego_motor_run_target"] = function (block, generator) {
    const port = block.getFieldValue("PORT");
    const speed = generator.valueToCode(block, "SPEED", Order.NONE) || "500";
    const target = generator.valueToCode(block, "TARGET", Order.NONE) || "90";
