...
export function registerUniversalGenerators(pythonGenerator: PythonGenerator) {
  ...
  pythonGenerator.forBlock["robot_drive_straight"] = function (block, generator) {
    ...
    if (profile === "xrp") {
      ...
      generator.definitions_["import_lego"] = "from pybricks.robotics import DriveBase\nfrom pybricks.pupdevices import Motor\nfrom pybricks.parameters import Port";
      generator.definitions_["lego_setup"] = "left_motor = Motor(Port.A)\nright_motor = Motor(Port.B)\nrobot = DriveBase(left_motor, right_motor, wheel_diameter=56, axle_track=114)";
  ...
  pythonGenerator.forBlock["robot_stop"] = function (block, generator) {
    const profile = getProfile();
    if (profile === "xrp") {
      generator.definitions_["import_drivetrain"] = "from XRPLib.differential_drive import DifferentialDrive";
      generator.definitions_["drivetrain_setup"] = "differentialDrive = DifferentialDrive.get_default_differential_drive()";
  ...
  pythonGenerator.forBlock["robot_motor_set_speed"] = function (block, generator) {
    ...
    if (profile === "xrp") {
      generator.definitions_["import_motor"] = "from XRPLib.encoded_motor import EncodedMotor";
      generator.definitions_[`motor${port}_setup`] = `motor${port} = EncodedMotor.get_default_encoded_motor(${port})`;
      return `motor${port}.set_speed(${speed})\n`;
    } else if (profile === "lego") {
      generator.definitions_[`motor${port}_setup`] = `motor_${port} = Motor(Port.${port})`;
  ...
  // --- Sensors ---
  pythonGenerator.forBlock["robot_get_distance"] = function (block, generator) {
    const profile = getProfile();
    if (profile === "xrp") {
      generator.definitions_["import_rangefinder"] = "from XRPLib.rangefinder import Rangefinder";
      generator.definitions_["rangefinder_setup"] = "rangefinder = Rangefinder.get_default_rangefinder()";
      return ["rangefinder.distance()", 0];
    } else if (profile === "lego") {
      generator.definitions_["import_ultra"] = "from pybricks.pupdevices import UltrasonicSensor";
      generator.definitions_["ultra_setup"] = "sonar = UltrasonicSensor(Port.S4)";
  ...
  pythonGenerator.forBlock["robot_get_reflectance"] = function (block, generator) {
    ...
    if (profile === "xrp") {
      generator.definitions_["import_reflectance"] = "from XRPLib.reflectance import Reflectance";
      generator.definitions_["reflectance_setup"] = "reflectance = Reflectance.get_default_reflectance()";
      const method = side === "LEFT" ? "get_left" : "get_right";
      return [`reflectance.${method}()`, 0];
    } else if (profile === "lego") {
      generator.definitions_["import_color"] = "from pybricks.pupdevices import ColorSensor";
      generator.definitions_[`color_${side}_setup`] = `color_${side.toLowerCase()} = ColorSensor(Port.S${side === 'LEFT' ? '1' : '2'})`;
      return [`color_${side.toLowerCase()}.reflection()`, 0];
    }
    return [`0 # Get Reflectance ${side} - Not implemented for ${profile}`, 0];
  };

  pythonGenerator.forBlock["robot_get_yaw"] = function (block, generator) {
    const profile = getProfile();
    if (profile === "xrp") {
      generator.definitions_["import_imu"] = "from XRPLib.imu import IMU";
      generator.definitions_["imu_setup"] = "imu = IMU.get_default_imu()\nimu.calibrate(1)";
